...
Once a dataset is submitted, its owner (the MassIVE user who launched the submission workflow) can [make it public](make_public.md) at any time by clicking the "Make Public" button near the top of the dataset's web page. Once a dataset is made public in this way, its password is removed and it then becomes visible in [public dataset searches](access_public_datasets.md).
