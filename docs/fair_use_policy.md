...
Consequently, **<ins>it has become necessary to require that all users limit their long-term (>=4 months) private user account storage to 100 GB or less, and that all private files are removed within 12 months of their last use in a data analysis workflow</ins>**.

Files being prepared for dataset submissions or used in data analysis workflows will still be able to temporarily use over 100 GB of storage, as is often required for submission of large datasets. We simply ask that you avoid keeping your account’s total storage above 100 GB for several months on end, and that you not leave unused files in your account for more than one year.

If the total volume of user account uploads exceeds 100 GB for more than 4 months or any inactive files remain for more than 12 months, then the account will be flagged as  "Non-compliant" and the following restrictions will apply:
...
- Non-compliant accounts will be able to reduce their storage use below the 100 GB limit and remove inactive files using either of the following two methods:
...
- If an account remains non-compliant for longer than 2 months (6 months total in non-compliant status), then
    - Old files not used in a data analysis workflow for >12 months will be automatically deleted.
